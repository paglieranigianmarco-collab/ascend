...
export default function LoanExtinguisher() {
    ...
    // Ensure we always show 2 loan slots
    const loanSlots = [0, 1].map(i => loans[i] || null);

    const saveLoan = async (loan, updates) => {
        try {
            if (loan && loan.id) {
    ...
    const createLoan = async (name) => {
        try {
            const res = await fetch('/api/loans', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, total_amount: 0, remaining_balance: 0, interest_rate: 0, base_payment: 0, extra_payment: 0 }),
            });
...
function LoanSection({ index, loan, onSave, onCreate }) {
    ...
    if (!loan) {
        return (
            <div className="card loan-card loan-empty" id={`loan-slot-${index}`}>
    ...
    return (
        <div className="card loan-card" id={`loan-${loan.id}`}>
            <div className="card-header">
                <h3><span className="icon">🏦</span> Loan {index + 1}</h3>
                <span className="loan-payoff-badge">
                    {monthsToPayoff > 0 ? `~${monthsToPayoff} mo` : '—'}
                </span>
            </div>
            ...
            <div className="loan-figures">
                <div className="input-group">
                    ...
                    <input type="number" value={form.total_amount} onChange={e => setForm(f => ({ ...f, total_amount: e.target.value }))} onBlur={e => handleBlur('total_amount', e.target.value)} placeholder="€0" />
                </div>
                <div className="input-group">
                    <label>Remaining Balance</label>
                    <input type="number" value={form.remaining_balance} onChange={e => setForm(f => ({ ...f, remaining_balance: e.target.value }))} onBlur={e => handleBlur('remaining_balance', e.target.value)} placeholder="€0" />
                </div>
                <div className="input-group">
                    <label>Interest Rate (%)</label>
                    <input type="number" step="0.01" value={form.interest_rate} onChange={e => setForm(f => ({ ...f, interest_rate: e.target.value }))} onBlur={e => handleBlur('interest_rate', e.target.value)} placeholder="0.00" />
                </div>
            </div>
            ...
            <div className="loan-progress">
                ...
                <div className="progress-bar-container">
                    <div className="progress-bar-fill" style={{ width: `${paidOff}%` }} />
                </div>
            </div>
            ...
            <div className="loan-payments">
                <div className="input-group">
                    <label>Base Payment / mo</label>
                    <input type="number" value={form.base_payment} onChange={e => setForm(f => ({ ...f, base_payment: e.target.value }))} onBlur={e => handleBlur('base_payment', e.target.value)} placeholder="€0" />
                </div>
                <div className="input-group">
                    <label>Extra Overpayment / mo</label>
                    <input type="number" value={form.extra_payment} onChange={e => setForm(f => ({ ...f, extra_payment: e.target.value }))} onBlur={e => handleBlur('extra_payment', e.target.value)} placeholder="€0" />
                </div>
            </div>
            ...
            <div className="chart-placeholder">
                ...
                <span>Accelerated Payoff Projection</span>
